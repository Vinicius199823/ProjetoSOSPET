...
try {
    ...
    if ($usuarioResult) {
        ...
        $queryUpdatePerdido = $conexao->prepare("UPDATE petperdido set encontrado = 1 where id_pet = :idPet");
        $queryUpdatePerdido->bindValue(':idPet', $idPet, PDO::PARAM_STR);
        $queryUpdatePerdido->execute();
    
        if ($PetResult) {
            $filename = $PetResult['filename'];

            $queryInsertPerdido = $conexao->prepare("
                INSERT INTO petencontrado (
                    id_usuario,
                    id_pet,
                    endereco,
                    dataEncontrado
